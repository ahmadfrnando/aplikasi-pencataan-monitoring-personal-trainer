    ...
    $(formSelector).on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $(modalSelector).find('[data-bs-dismiss="modal"]').trigger('click');
                $(tableSelector).load(location.href + ' ' + tableSelector + ' > *');
                    toastMixin.fire({
                        animation: true,
                        title: successMessage || `${response.message}`,
                    });
            ...
            error: function(xhr) {
                console.log('berhasil-error');
                if (xhr.status === 422) {
                    let res = xhr.responseJSON;
                    let errorMessages = Object.values(res.errors).flat().join('\n');
                     toastMixinError.fire({
                        animation: true,
                        title: errorMessages || `${response.message}`,
                    }).then(() => {
...
function initializeDataTableParams(tableSelector, ajaxRoute, columnsConfig) {
  var table = $(tableSelector).DataTable({
    destroy: true,
    responsive: true,
    processing: true,
  ...
  // Custom pagination icons
  table.on('draw', function() {
    $('#DataTables_Table_0_previous a').html(' <i class="ti ti-arrow-left"></i>');
    $('#DataTables_Table_0_next a').html(' <i class="ti ti-arrow-right"></i>');
  });
...
function regenerateAjax(url, table) {
    Swal.fire({
        title: 'Apakah anda yakin?',
        text: "Data akan di generate ulang!",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
