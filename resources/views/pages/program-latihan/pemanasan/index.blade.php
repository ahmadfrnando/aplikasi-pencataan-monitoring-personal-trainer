    ...
    <div class="col-12">
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h3 class="card-title fw-semibold text-center">Semua Latihan Pemanasan</h3>
                <button type="button" data-bs-toggle="modal" data-bs-target="#tambahLatihanPemanasan" data-id="{{ $program->id }}" class="btn btn-primary justify-content-center text-white text-center">
                    <i class="ti ti-plus me-2"></i>
                    Tambah Latihan Pemanasan
                </button>
            </div>
            <div class="card-body">
                <div class="table-responsive mt-4">
                    <table id="table-pemanasan" class="table mb-0 text-nowrap varient-table align-middle fs-3">
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            @if(!$program->program_pemanasan->isEmpty())
                            @foreach ($program->program_pemanasan as $key => $value)
                            <tr>
                                ...
                                <td class="px-0 justify-content-center gap-2 d-flex">
                                    <button id="btnEdit" data-bs-toggle="modal" data-bs-target="#ubahLatihanPemanasan-{{ $value->id }}" data-id="{{ $value->id }}" class="btn btn-sm btn-primary"><i class="ti ti-edit"></i></button>
                                    <button id="btnDelete" data-id="{{ $value->id }}" class="btn btn-sm btn-danger"><i class="ti ti-trash"></i></button>
                                </td>
                                @include('pages.program-latihan.pemanasan.edit-modal-form')
                            </tr>
                            @endforeach
                            @else
                            <tr>
                                <td colspan="4" class="text-center">
                                    <figure><img src="{{ asset('assets/images/illustrator/personal-trainer.svg') }}" alt="pt" width="250" class="img-fluid">
                                        <figcaption class="text-muted mt-3">Belum ada latihan pemanasan</figcaption>
                                    </figure>
                                </td>
                            </tr>
                            @endif
                ...
                @include('pages.program-latihan.pemanasan.create-modal-form')
            </div>
        </div>
    </div>
</div>
...
<script type='text/javascript'>
    $(function() {
        let table = '#table-pemanasan';
        let formSelector = '#submitForm';
        let modal = '#tambahLatihanPemanasan';
        ...
        $(document).on('click', '#btnEdit', function() {
            ...
            let id = $(this).data('id');
            let actionUrlEdit = "{{ route('program.pemanasan.update', ':id') }}";
        ...
        $(document).on('click', '.checkbox', function() {
            ...
            $.ajax({
                url: '{{ route("program.pemanasan.update-isdone") }}', // Route to handle saving the data
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    id: id,
                ...
                success: function(response) {
                    $('#table-pemanasan').load(location.href + ' #table-pemanasan > *');
                },
        ...
        $(document).on('click', '#btnDelete', function() {
            var id = $(this).data('id');
            var route = "{{ route('program.pemanasan.destroy', ':id') }}";
            route = route.replace(':id', id);
            var table = '#table-pemanasan';
